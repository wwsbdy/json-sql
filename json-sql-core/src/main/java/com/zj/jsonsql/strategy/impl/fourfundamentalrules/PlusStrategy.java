 ...
 * @date : 2025/3/31
 */
public class PlusStrategy implements IFunctionStrategy {
    ...
    public Object get(Row row, SqlBasicCall sqlBasicCall) {
        ...
        if (NumberUtils.isCreatable(param1Str) && NumberUtils.isCreatable(param2Str)) {
            return new BigDecimal(param1Str).add(new BigDecimal(param2Str));
        }
        return param1Str + param2Str;
    }

    @Override
    public FuncEnum getType() {
        return FuncEnum.PLUS;
    }
